...
import { fetchCafeById, createCafe, updateCafe } from '@/store/slices/cafe-slice';
import { showNotification, setFormDirty } from '@/store/slices/ui-slice';
import { getLogoUrl, getFileUploadUrl, deleteLogoFile } from '@services/api-service';
...
const AddEditCafePage = () => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const temporaryFileName = `temp_${Date.now()}_${Math.random().toString(36).substring(2, 9)}.${fileExtension}`;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isEditing && id) {
        ...
        const isLogoChanged = formData.logo && oldLogoFileName && oldLogoFileName !== '';
        
        savedCafeData = await dispatch(updateCafe({ formData: cafeFormData })).unwrap();
      ...
      if (formData.logo && formData.logoFileName && savedCafeData) {
        try {
          ...
          if (data.fileName !== formData.logoFileName) {
            ...
            await dispatch(updateCafe({ 
              formData: updateLogoFormData 
            })).unwrap();
            
            if (isEditing && oldLogoFileName && !oldLogoFileName.startsWith('temp_')) {
              setTimeout(async () => {
                try {
                  await deleteLogoFile(oldLogoFileName);
                } catch (error) {
          ...
          console.error('Warning: Logo upload failed after cafe was saved:', error);
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <FormTextField
                  ...
                  helperText={errors.description || `${formData.description.length}/256 characters`}
                  multiline
                  rows={4}
              ...
              <Grid item xs={12}>
                ...
                <label htmlFor="logo-upload">
                  ...
                  >
                    {isUploading ? 'Uploading...' : formData.logoFileName ? 'Change Logo' : 'Upload Logo'}
                  </Button>
                </label>
