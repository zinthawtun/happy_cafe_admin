...
const AddEditEmployeePage = () => {
  ...
  const { list: cafes, loading: cafesLoading } = useAppSelector(
  ...
  useEffect(() => {
    if (isEditing && selectedEmployee && cafes.length > 0) {
      const matchingCafe = cafes.find(
        (cafe) => cafe.name === selectedEmployee.cafe
      );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        typeof error === "string" ? error : "Failed to process employee data";

      if (
        errorMessage.includes("email address or phone number already exists")
      ) {
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <FormTextField
                  ...
                  helperText={
                    errors.phoneNumber ||
                    "Must start with 8 or 9 and have 8 digits"
                  }
              ...
              <Grid item xs={12}>
                <FormControl fullWidth margin="normal" error={!!errors.cafeId}>
                  ...
                  {errors.cafeId && (
                    <FormHelperText>{errors.cafeId}</FormHelperText>
                  )}
                </FormControl>
              </Grid>
